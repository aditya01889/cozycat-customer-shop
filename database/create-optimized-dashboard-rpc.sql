...
BEGIN
  ...
  SELECT 
    ...
    (SELECT json_agg(
      ...
      SELECT id, customer_name, total_amount, status, created_at, email
      FROM orders 
      WHERE status != 'cancelled' 
      AND created_at >= COALESCE(start_date::timestamp, '1900-01-01'::timestamp)
...
-- 5. Customer analytics with pagination (simplified without user_id dependency)
...
AS $$
DECLARE
  previous_period_revenue decimal := 0;
BEGIN
  RETURN QUERY
