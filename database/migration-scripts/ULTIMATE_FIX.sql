-- ULTIMATE FIX FOR ALL OPERATIONS PAGE ISSUES
...
-- ==========================================
-- STEP 2: Add sample data to empty tables (SIMPLIFIED)
-- ==========================================

-- Add sample production_batches with valid product relationships
-- Remove ON CONFLICT clause to avoid constraint issues
...
SELECT 
    ...
    NOW() as updated_at,
    o.id as order_id,
    NULL as start_time,  -- Make this nullable
    NOW() as end_time,
    1 as priority,
...
-- Add sample deliveries (simplified)
...
-- Add sample vendors (if table exists) - simplified
DO $$
BEGIN
    IF EXISTS (
...
-- Add sample inventory (if table exists) - simplified
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
