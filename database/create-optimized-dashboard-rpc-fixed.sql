...
-- Phase 3.1: API Layer Optimization - FIXED VERSION
-- Only uses columns that we know exist in the database
...
-- 4. Recent activity with pagination and filtering (simplified)
...
-- 5. Customer analytics with pagination (simplified version)
...
BEGIN
  ...
  ) o ON p.full_name = o.customer_name
...
-- 7. Inventory analytics (simplified)
...
-- Create indexes for performance (only for columns we know exist)
