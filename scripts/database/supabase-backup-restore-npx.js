...
/**
 * Supabase CLI Backup and Restore (using npx)
 * Uses npx supabase to backup production and restore to staging
 */
...
console.log('🔄 Supabase CLI Backup & Restore (npx)');
console.log('=====================================');
...
async function backupAndRestore() {
  try {
    ...
    // Backup production database using npx
    try {
      ...
      console.log(`🔧 Running: npx supabase db dump --db-url=${prodUrl} --file=${backupFile}`);
      
      // Create a temporary .env file for the backup
      const tempEnv = `SUPABASE_DB_PASSWORD=${prodKey}`;
      const tempEnvFile = path.join(__dirname, '../../temp-backup.env');
      fs.writeFileSync(tempEnvFile, tempEnv);
      
      execSync(`npx supabase db dump --db-url="${prodUrl}" --file="${backupFile}"`, { 
        stdio: 'inherit',
      ...
      try {
        ...
        const prodConnectionString = `postgresql://postgres:${prodKey}@aws-0-us-east-1.pooler.supabase.co:5432/postgres`;
    ...
    try {
      ...
      // Create connection string for staging
      const stagingConnectionString = `postgresql://postgres:${stagingKey}@aws-0-us-east-1.pooler.supabase.co:5432/postgres`;
      ...
      try {
        ...
        execSync(`psql "${stagingConnectionString}" < "${backupFile}"`, { stdio: 'inherit' });
